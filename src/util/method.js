...
export let validation = {
    ...
    isContainLetterAndNumber: (value, name, content) => {
    ...
    isEmailValid: (value, name, content) => {
        let regexEmail =
            /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/;
    ...
    isNumberAndValueValid: (value, name, content, minValue, maxValue) => {
        value = Number(value);
        let regexNumber = /^[0-9]+$/;
    ...
    isLengthValid: (value, name, minLength, maxLength) => {
    ...
    isValueValid: (value, name, minValue, maxValue) => {
    ...
    isPassValid: (value, name, min, max) => {
        let regexPattern = `(?=.*\\d)(?=.*[A-Z])(?=.*[ \`!@#$%^&*()_+\\-=\\[\\]{};':"\\\\|,.<>\\/?~]).{${min},${max}}`;

        let regexPassword = new RegExp(regexPattern);
        ...
        if (regexPassword.test(value)) {
            ...
            document.querySelector(
                `#${name}`
            ).innerHTML = `Mật khẩu không được bỏ trống, phải chứa ít nhất 1 số, 1 chữ in hoa, 1 ký tự đặc biệt và độ dài từ ${min}-${max} ký tự !`;
            return false;
        }
    },

    isDateValid: (value, name, content) => {
    ...
    isNumberAndLengthValid: (value, name, content, minLength, maxLength) => {
        let length = value.length;
        let regexNumber = /^[0-9]+$/;
        if (
...
export let getStorageJSON = (name) => {
 ...
 */
export let findMostCommonString = (arrstrings) => {
 ...
 */
export let findHighestAndLowest = (array) => {
    let indexMax = 0;
 ...
 */
export let recursiveReverse = (str) => {
